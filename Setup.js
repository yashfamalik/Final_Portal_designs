...
function mountSetup(container, props = {}) {
    ensureSetupStyles();
    ...
    // Function to equalize column heights (simplified for CSS Grid)
    function equalizeColumnHeights() {
        // CSS Grid handles equal heights automatically with grid-auto-rows: 1fr
        // This function is kept for any future adjustments if needed
        setTimeout(() => {
            // Force a reflow to ensure grid calculations are complete
...
(function () {
  function autoMount() {
    if (window.__CLAIM_SETUP_MOUNTED__) return;
    window.__CLAIM_SETUP_MOUNTED__ = true;

    var host = document.getElementById("claim-setup-root");
    if (!host) {
      host = document.createElement("div");
      host.id = "claim-setup-root";
      document.body.appendChild(host);
    }

    if (typeof window.mountSetup === "function") {
      window.mountSetup(host, {});
      return;
    }

    if (typeof mountSetup === "function") {
      window.mountSetup = mountSetup;
      mountSetup(host, {});
      return;
    }

    if (window.ClaimSetup && typeof window.ClaimSetup.init === "function") {
      window.ClaimSetup.init(host, {});
    }
  }
