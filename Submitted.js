...
(function () {
  function autoMount() {
    if (window.__CLAIM_SUBMITTED_MOUNTED__) return;
    window.__CLAIM_SUBMITTED_MOUNTED__ = true;

    var host = document.getElementById("claim-submitted-root");
    if (!host) {
      host = document.createElement("div");
      host.id = "claim-submitted-root";
      document.body.appendChild(host);
    }

    if (typeof window.mountSubmitted === "function") {
      window.mountSubmitted(host, {});
    } else if (window.ClaimSubmitted && typeof window.ClaimSubmitted.init === "function") {
      window.ClaimSubmitted.init(host, {});
    }
  }
